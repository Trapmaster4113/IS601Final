...
def test_model_exponent():
    dummy = uuid4()
    calc = Calculation.create("exponent", dummy, [1,5])
    result = calc.get_result()
    assert result == 1, f"Exponent result incorrect: expected 64, got {result}"

    #Test exponent with negative power error
    with pytest.raises(ValueError):
        calc_negexp = Calculation.create("exponent", dummy, [1-1])
        calc_negexp.get_result()